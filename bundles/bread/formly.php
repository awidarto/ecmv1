...
class Formly
{
	...
	public function checkbox($name, $label = '', $value = 1, $checked = false, $attributes = array())
	{
		...
		return $this->build_wrapper($field, $name, $label,$checked, true, false);
	}
	 ...
	 * @return string
	 */
	private function build_wrapper($field, $name, $label = '', $checked = false ,$checkbox = false, $radio = false)
	{
		...
		// test
		//$error = "error";
		//$this->display_inline_errors = true;
	 ...
	 * @return string
	 */
	private function build_label($name, $label = '',$required = false, $attributes = array())
	{
		$out = '';
		if ( ! empty($label))
		{
